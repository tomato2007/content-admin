...
- `Этап 1` реализован в основном: есть доменные сущности, доступы, Filament-кабинет аккаунта, seed платформ и аудит базовых действий.
- `Этап 2` реализован в основном: есть `PostingPlan`, `PostingSlot`, preview ближайших слотов и редактирование плана.
- `Этап 3` реализован в основном: есть `planned_posts`, moderation workflow, replace/delete/reschedule и audit log.
- `Этап 4` реализован частично: есть dry-run, ручной publish, idempotency и запись в `posting_history`, но нет завершённого автоматического контура публикации.
- `Этап 5` начат частично: есть интерфейс драйвера и Telegram-specific интеграция, но нет полноценного platform-aware resolver и новых платформ.
- До продолжения новой функциональности нужен отдельный стабилизационный этап, иначе разработка пойдёт поверх сломанного quality gate, несогласованного окружения и machine-specific интеграций.

## Этап 0. Стабилизация перед продолжением реализации
Цель: сделать проект проверяемым, переносимым и архитектурно определённым.

Сделать:
- quality gate и test bootstrap:
  - создать `tests/Unit` или убрать пустой suite из `phpunit.xml`
  - добиться прохождения `php artisan test`
  - привести код к прохождению `vendor/bin/pint`
  - зафиксировать рабочий тестовый драйвер БД и описать его в dev setup
- выравнивание требований окружения:
  - синхронизировать `composer.json`, `.env.example`, `README_DEV.md`, `AGENTS.md`
  - устранить конфликт по `PHP 8.3+`, `PostgreSQL`, `Pest`/`PHPUnit`
  - зафиксировать поддерживаемый локальный сценарий запуска через Docker
- dev bootstrap и документация:
  - добавить сидер локального admin-пользователя или убрать несуществующие креды из README
  - описать обязательные env-переменные для Filament и publishing integration
  - обновить README под фактический процесс запуска
- архитектурный контракт:
  - добавить `docs/architecture.md`, `docs/domain.md`, `rules/rules.md`
  - описать, как текущая структура маппится на feature-first и DDD
  - зафиксировать правила для новых модулей, actions и Filament-компонентов
- переносимость publishing integration:
  - вынести абсолютный путь к Python script в config/env
  - изолировать доступ к `telegram_runtime` через adapter/validator layer
  - подготовить fake/stub integration для тестов и локальной разработки

Результат:
- проект воспроизводимо поднимается и проверяется в одном предсказуемом сценарии
- новая функциональность не ломает базовые проверки
- команда работает от одного архитектурного и инфраструктурного контракта
...
## План продолжения от текущего состояния

Ниже план уже не с нуля, а от текущей кодовой базы.

### Следующие шаги по Этапу 0
1. Починить test bootstrap и quality gate.
2. Привести в соответствие `composer.json`, `.env.example`, `README_DEV.md`, docker-сценарий и фактические требования проекта.
3. Добавить локальный bootstrap для входа в админку и базовой разработки.
4. Формализовать missing docs по архитектуре, домену и правилам разработки.
5. Убрать machine-specific зависимости из publishing integration.

### Следующие шаги по Этапу 1
1. Ограничить доступ в Filament не любым аутентифицированным пользователям, а только допустимым ролям/операторам.
2. Довести локальные сиды до состояния, при котором разработчик после `migrate --seed` сразу видит рабочий кабинет.
3. Разнести ответственность между account management, moderation и publishing так, чтобы новые изменения шли в feature-first модули, а не в общий слой `app/Services`.
4. Дофиксировать audit-покрытие для управления администраторами, настройками аккаунта и системных действий.

### Следующие шаги по Этапу 2
1. Уйти от raw JSON как основного UI для `rules` и оставить JSON только как внутреннее хранилище.
2. Добавить явную валидацию timezone, quiet hours и slot-конфликтов.
3. Сделать более полезный overview плана: ближайшие окна публикации, quiet hours, активные ограничения.
4. Подготовить базу для генерации planned posts из плана, а не только ручного управления.

### Следующие шаги по Этапу 3
1. Вынести moderation transitions в отдельные action-классы с чёткими guard conditions.
2. Зафиксировать допустимые переходы статусов и ошибки домена в одном месте.
3. Улучшить UX очереди: группировка по статусам, pending buckets, быстрые bulk-фильтры.
4. Добавить тесты не только на happy path, но и на запрещённые переходы, повторные действия и race-like сценарии.

### Следующие шаги по Этапу 4
1. Добавить `schedule`/`command`/`job` для автоматической публикации запланированных постов.
2. Вынести idempotency и locking в отдельный orchestration layer, а не держать только внутри service метода.
3. Унифицировать историю попыток: `manual`, `retry`, `scheduled`, `dry-run`.
4. Отделить Telegram runtime validation от UI-страниц и покрыть publish flow contract-тестами.

### Следующие шаги по Этапу 5
1. Сделать resolver драйвера по `platform.driver`, а не глобальный выбор драйвера на всё приложение.
2. Разделить platform-specific settings forms и validators по платформам.
3. Подготовить skeleton drivers для `VK` и `X` без переписывания core publishing flow.
4. Спроектировать переход от Telegram-specific content source к универсальному content domain.

---

## Что делать прямо сейчас, без распыления

### Трекер выполнения Sprint 0
- `[x]` Шаг 1. Восстановить quality gate: test bootstrap, `tests/Unit`, базовый запуск `php artisan test`
- `[x]` Шаг 2. Привести код к прохождению `vendor/bin/pint`
- `[x]` Шаг 3. Синхронизировать окружение и документацию: `composer.json`, `.env.example`, `README_DEV.md`
- `[x]` Шаг 4. Добавить локальный bootstrap для входа в админку
- `[x]` Шаг 5. Подготовить архитектурные документы и правила разработки
- `[x]` Шаг 6. Убрать machine-specific зависимости из publishing integration

### Sprint 0 (обязателен перед новой функциональностью)
- закрыть `Этап 0` целиком
- добиться прохода тестов и форматирования
- синхронизировать окружение, документацию и локальный bootstrap

### Sprint 1
- взять следующие шаги по `Этапу 1`
- ограничить доступ в Filament
- довести seed/bootstrap до рабочего локального состояния
- зафиксировать архитектурные документы и правила для новых модулей

### Трекер выполнения Sprint 1
- `[x]` Шаг 1. Ограничить доступ в Filament не любым аутентифицированным пользователям, а только допустимым операторам
- `[x]` Шаг 2. Довести seed/bootstrap до состояния рабочего локального кабинета после `migrate --seed`
- `[x]` Шаг 3. Разнести ответственность между account management, moderation и publishing для следующих модульных изменений
- `[x]` Шаг 4. Дофиксировать audit-покрытие для управления администраторами, настройками аккаунта и системными действиями

### Sprint 2
- взять следующие шаги по `Этапам 2-3`
- довести structured UI и validation для `PostingPlan`
- усилить queue/moderation transitions и тесты на негативные сценарии

### Трекер выполнения Sprint 2
- `[x]` Шаг 1. Уйти от raw JSON как основного UI для PostingPlan rules и оставить JSON только как внутреннее хранилище
- `[x]` Шаг 2. Добавить явную валидацию timezone, quiet hours и slot-конфликтов
- `[x]` Шаг 3. Сделать более полезный PostingPlan overview
- `[x]` Шаг 4. Усилить queue/moderation transitions и негативные сценарии

### Sprint 3
- взять следующие шаги по `Этапу 4`
- добавить scheduler/job/command для автоматической публикации
- вынести idempotency и locking в отдельный orchestration layer

### Трекер выполнения Sprint 3
- `[x]` Шаг 1. Добавить scheduler, command и queue job для автоматической публикации due planned posts
- `[x]` Шаг 2. Вынести idempotency и locking в отдельный orchestration layer

### Sprint 4
- взять следующие шаги по `Этапу 5`
- внедрить platform-aware driver resolver
- разделить forms/validators по платформам
- подготовить каркас для `VK` и `X`

### Трекер выполнения Sprint 4
- `[x]` Шаг 1. Внедрить platform-aware driver resolver вместо глобального выбора драйвера на всё приложение
- `[x]` Шаг 2. Разделить forms и validators по платформам
- `[x]` Шаг 3. Подготовить каркас driver-level интеграции для `VK` и `X`

### Sprint 6
- проверить и расширить реальный Telegram publishing flow
- добавить UI-подключение Telegram-бота через токен
- связать подключённого бота с аккаунтом публикации

### Трекер выполнения Sprint 6
- `[x]` Шаг 1. Добавить UI-поток подключения Telegram-бота через токен с валидацией токена и сохранением привязки к platform account
- `[x]` Шаг 2. Перевести Telegram publish-flow на прямую отправку через Bot API с использованием подключённого bot token и `settings.target_chat_id`, сохранив legacy fallback через Python adapter

### Sprint 7
- реализовать production-style источник контента из `anecdots.public.posts`
- ввести явные правила формирования очереди из source pool
...
- синхронизировать user-facing статусы очереди с доменными enum
...
- Telegram publishing поддерживает direct Bot API path, а Python adapter остаётся legacy fallback

Это даст быстрый прогресс без архитектурного тупика.


## последняя сессия 019dab2b-32ab-7f40-87e7-17ead58963b9